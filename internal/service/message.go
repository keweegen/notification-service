...
func (m *Message) HandleMessages(ctx context.Context, quit <-chan bool) {
...
func (m *Message) handleChannelMessages(ctx context.Context, quit <-chan bool, channel channel.Channel) {
    for {
        select {
        case <-quit:
            return
